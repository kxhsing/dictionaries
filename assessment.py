...
def translate_to_pirate_talk(phrase):
    ...
    for word in words:
        if word in pirate_dict:
            word_list.append(pirate_dict[word])
        else:
            word_list.append(word)
...
def kids_game(names):
    ...
    for word in names[1:]:
        if word[0] in dict_first_letter:
    ...
    next_word = names[0]
    output_list = [next_word]

    while True:
        if next_word[-1] in dict_first_letter and len(dict_first_letter[next_word[-1]]):
            next_word = dict_first_letter[next_word[-1]][0]
            output_list.append(next_word)

            dict_first_letter[next_word[0]].remove(next_word)
